...
2. DGPS status added to information banner. **NB** this indicates the successful reception of DGPS correction data (e.g. NTRIP or SBAS) based on information from NMEA (GGA, GNS) or UBX (NAV-PVT, NAV-STATUS, RXM-RTCM) messages. It does *not* necessarily reflect the quality of the incoming data. DGPS corrections typically expire 60 seconds after loss of correction data. Note that a) NMEA and UBX messages do not always give consistent indications of DGPS status, and b) DGPS status *cannot* be reliably inferred from other NMEA message types (e.g. RMC, VTG) earlier than NMEA 4.10. 
