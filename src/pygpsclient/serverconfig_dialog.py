...
class ServerConfigDialog(ToplevelDialog):
    ...
    def _reset(self):
        ...
        if basemode == "DISABLED":  # legacy settings
            ...
            basemode = BASE_FIXED
        elif basemode == "SURVEY IN":
            basemode = BASE_SVIN
        self.base_mode.set(basemode)
        cfg.set("ntripcasterbasemode_s", basemode)
        self._on_update_basemode(None, None, TRACEMODE_WRITE)
        receiver_type = cfg.get("ntripcasterrcvrtype_s")
        if receiver_type == "Quectel LC29H":  # legacy settings
            receiver_type = "Quectel LC Series"
        elif receiver_type == "Quectel LG29P/LG580P":
            receiver_type = "Quectel LG Series"
        elif receiver_type == "Unicore UM98n":
            receiver_type = "Unicore UM9* Series"
        elif receiver_type == "Septentrio Mosaic X3/X5":
            receiver_type = "Septentrio Mosaic Series"
        self.receiver_type.set(receiver_type)
        cfg.set("ntripcasterrcvrtype_s", receiver_type)
    ...
    def _on_socketserve(self, var, index, mode):
        ...
        if self.receiver_type.get() in (UBLOX_ZEDF9, UBLOX_ZEDX20):
            ntriprtcmstr = (
                "1002(5),1006(5),1010(5),1077(1),1087(1),1097(1),1127(1),1230(1)"
            )
        elif self.receiver_type.get() == UNICORE_UM980:
            ntriprtcmstr = "1002(10),1006(10),1012(10),1019(10),1020(10),1033(10),1074(1),1084(1),1094(1),1124(1)"
        elif self.receiver_type.get() == QUECTEL_LCSERIES:
            ntriprtcmstr = "1005(10),1012(10),1019(10),1020(10),1033(10),1042(10),1046(10),1077(1),1087(1),1097(1),1127(1)"
        elif self.receiver_type.get() == QUECTEL_LGSERIES:
            ntriprtcmstr = "1005(10),1012(10),1019(10),1020(10),1033(10),1077(1),1087(1),1097(1),1127(1),1137(1)"
        elif self.receiver_type.get() == SEPTENTRIO_MOSAIC:
            ntriprtcmstr = "1006(5),1013(5),1019(5),1020(5),1033(5),1077(1),1087(1),1097(1),1127(1),1230(1)"
        else:
            ntriprtcmstr = RTCMSTR
        if self._socket_serve.get():
            # start server
            self.__app.sockserver_start(ntriprtcmstr)
            self._fixed_lat_temp = self.fixedlat.get()
            self._fixed_lon_temp = self.fixedlon.get()
            self._fixed_hae_temp = self.fixedhae.get()
        else:  # stop server
