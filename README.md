...
|![custom map](https://github.com/semuconsulting/PyGPSClient/blob/master/images/custommap.png?raw=true)| Map Type = 'custom': one or more user-defined custom offline maps can be imported by setting the `usermaps_l` field in the json configuration file. The `usermaps_l` setting represents a list of map paths and bounding boxes in the format [fully qualified path to a map image (*image must be a [supported format](https://pillow.readthedocs.io/en/stable/handbook/image-file-formats.html) and use a standard WGS84 Web Mercator projection e.g. EPSG:4326*), calibration (bounding box) coordinates [top left lat, top left lon, bottom right lat, bottom right lon]] - see [example configuration file](https://github.com/semuconsulting/PyGPSClient/blob/master/pygpsclient.json). A utility [`georef.py`](https://github.com/semuconsulting/PyGPSClient/blob/master/examples/georef.py) is provided in the \examples folder which automatically generates the necessary calibration settings for a georeferenced image file e.g. geoTIFF. |
