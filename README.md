...
|![static map](https://github.com/semuconsulting/PyGPSClient/blob/master/images/staticmap.png?raw=true)| Static offline Mercator world map showing current global location.
|![custom map](https://github.com/semuconsulting/PyGPSClient/blob/master/images/custommap.png?raw=true)| A user-defined custom offline map can be imported by setting the `usermappath_s` and `usermapcalibration_l` fields in the json configuration file and selecting 'custom'. The `usermappath_s` setting represents the fully qualified path to a map image (*image must be a [supported format](https://pillow.readthedocs.io/en/stable/handbook/image-file-formats.html) and use a standard WGS84 Web Mercator projection e.g. EPSG:4326*); the `usermapcalibration_l` setting represents a list of calibration (bounding box) coordinates [top left lat, top left lon, bottom right lat, bottom right lon] - see [example configuration file](https://github.com/semuconsulting/PyGPSClient/blob/master/pygpsclient.json). A utility [`georef.py`](https://github.com/semuconsulting/PyGPSClient/blob/master/examples/georef.py) is provided in the \examples folder which automatically generates the necessary calibration settings for a georeferenced image file e.g. geoTIFF. |
|![webmap widget](https://github.com/semuconsulting/PyGPSClient/blob/master/images/webmap_widget.png?raw=true)| Dynamic, online web map via MapQuest API (*requires an Internet connection and free [Mapquest API Key](#mapquestapi)*). Left Click +/- to zoom in or out. Right click +/- to zoom in or out to maximum extent. By default, the web map will automatically refresh every 60 seconds (*indicated by a small timer icon at the top left*). The default refresh rate can be amended by changing the `"mapupdateinterval_n":` value in your json configuration file, but **NB** the facility is not intended to be used for real-time navigation. Double-click anywhere in the map to immediately refresh. |
...
|![scatterplot widget](https://github.com/semuconsulting/PyGPSClient/blob/master/images/scatterplot_widget.png?raw=true)| Scatterplot widget showing variability in position reporting over time. Uses a haversine great circle formula at scales greater than 50m, and a planar approximation formula for smaller scales. Double-click to clear existing plot. |
