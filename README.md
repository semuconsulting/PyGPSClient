...
|![scatterplot widget](https://github.com/semuconsulting/PyGPSClient/blob/master/images/scatterplot_widget.png?raw=true)| Scatterplot widget showing variability in position reporting over time. Check Autorange to automatically set plot range. Select Average to center plot on dynamic average position (*displayed at top left*), or Fixed to center on fixed reference position (*if entered*). Use the range slider or mouse wheel to adjust plot range. Right-click to set fixed reference point to the current mouse cursor position. Double-click clear existing plot. The default autorange, center, scale index and fixed reference position settings can be stored in the json configuration file as `scatterautorange_b`/`scattercenter_s`/`scatterscale_n`/`scatterlat_f`/`scatterlon_f`. |
