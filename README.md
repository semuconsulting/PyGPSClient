...
|![static map](https://github.com/semuconsulting/PyGPSClient/blob/master/images/staticmap.png?raw=true)| Static Mercator world map showing current global location. The world map can be substituted for a user-defined static map by setting the `usermappath_s` and `usermapcalibration_l` fields in the configuration file. The `usermappath_s` setting represents the fully qualified path to a map image (*map must use a standard WGS84 Web Mercator projection*); the `usermapcalibration_l` setting represents a list of calibration coordinates [top left lat, top left lon, bottom right lat, bottom right lon] - see example configuration file. |
